...
#Load dữ liệu 
alcohol <- read.csv("../lab_02/student-alcohol.csv")
View(alcohol)
...
#Điền giá trị "other" cho dòng 56 (Mjob bị thiếu)
alcohol$Mjob[53] <- "other"
...
alcohol$school <- factor(alcohol$school,
                         levels= c("GP","MS"),
                         labels = c("Gabriel Perecia","Mousino da Silveira")
                         )
...
alcohol$sex <- factor(alcohol$address,
                      levels= c("R","U"),
...
alcohol$sex <- factor(alcohol$famsize,
                      levels= c("GT3","LE3"),
                      labels = c("more than 3","less or equal to 3")
)
#Parent's cohabitation status - tình trạng chung sống của chu
...
# 7. Lưu dữ liệu sạch
write.csv(alcohol, "../lab_02/student-alcohol-cleaned.csv", row.names = FALSE)

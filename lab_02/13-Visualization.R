...
# Nhưng khi vẽ ra sẽ thấy khác nhau !
...
#Biểu đồ cột được sử dung để hiển thị dữ liệu phân loại(catergor)
# Ví dụ: Đếm số lương xe theo số xy-lanh

#Sử dụng dữ liệu mtcars
data("mtcars")
View(mtcars)

#Đếm số lương xe theo số động cơ 
...
hist(exam_scores,
     breaks = 10,
     col = "blue",
     main = "Phân bố điểm thi",
     xlab = "Điểm")

# Đường trung bình
abline(v = mean(exam_scores), col = "red", lwd = 2) #v:vẽ đường dọc, lwd: độ dày
...
colors <- c("red","blue","green")[mtcars$cyl]
...
plot(mtcars$hp, mtcars$mpg,
     ...
     xlab = "Horsepower",
     ylab = "MPG",
     main = "hp vs mpg") #hp: trục X, #mpg: trục Y, pch: chấm tròn 
...
par(mfrow = c(2,2)) #chia màn hình thành 2x2

# Histogram
hist(mtcars$hp, col="lightblue", main="Histogram of hp")

# Boxplot
boxplot(mtcars$hp, col="orange", main="Boxplot of hp")
...
sales <- rbind(Q1,Q2,Q3,Q4) #rbind: ghép theo hàng( mỗi quý 1 hàng)

# Grouped bar chart
barplot(sales,
        beside=TRUE,#cột đứng cạnh nhau
        col=c("red","blue","green","orange"),
        names.arg=products,
        legend=TRUE, # hiện chú thích 
        main="Doanh thu theo quý") 

# Line plot
matplot(t(sales), type="l", lty=1,
        col=c("red","blue","green","orange"),
        xlab="Quarter",
        ylab="Sales",
        main="Doanh thu theo sản phẩm")

legend("topleft", legend=products,
       col=c("red","blue","green","orange"), lty=1)

# Tổng doanh thu mỗi quý
total_quarter <- colSums(sales) #cộng theo cột (Q1->Q4)
...
# Grouped bar
barplot(sales, beside=TRUE, col=rainbow(4), main="Grouped Bar")

# Line plot
matplot(t(sales), type="l", lty=1, col=rainbow(4), main="Line Plot")

# Pie chart
pie(total_quarter, main="Doanh thu mỗi quý")

# Bar theo sản phẩm
barplot(rowSums(sales), col="green", main="Doanh thu mỗi sản phẩm")
# ==============================================================================
# TÀI LIỆU THAM KHẢO
# ==============================================================================

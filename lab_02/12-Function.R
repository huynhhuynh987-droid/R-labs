...
greet("Thư")                    # "Xin chào Thư !"
greet("Thư", "Chào buổi sáng")  # "Chào buổi sáng Thư !"
...
average_score <- function(scores){
  ...
  if(any(scores < 0 | scores > 10)){
    stop("Điểm phải nằm trong khoảng 0-10")
  }
  
  return(mean(scores))
}

average_score(c(7,8,9,NA))
# ------------------------------------------------------------------------------
...
classify_student <- function(score){
  ...
  return(list(Grade=grade,GPA=gpa))
}
...
net_salary <- function(base_salary, allowance, work_days, overtime_hours){
  ...
  total_salary <- base_salary + allowance + overtime_pay
...
net_salary(8000000,1000000,26,10)
